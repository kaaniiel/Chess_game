...
function createGame() {
    ...
    fetch(`api.php?action=create&name=${encodeURIComponent(name)}`)
        .then(r=>r.json())
        .then(d => {
...
function joinGame(roomId = null) { 
    ...
    fetch(`api.php?action=join&roomId=${c}&name=${encodeURIComponent(inputName)}`)
        .then(r=>r.json())
        .then(d => { 
...
function leaveLobby() {
    showConfirm("Quitter la partie ?", function() {
        if(myRoomId && myName) {
            fetch(`api.php?action=leave&roomId=${myRoomId}&name=${encodeURIComponent(myName)}`)
                .finally(() => {
...
function cancelGame() { 
    showConfirm("Annuler la partie ?", () => { 
        fetch(`api.php?action=cancelGame&roomId=${myRoomId}`); 
    }); 
}
