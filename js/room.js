...
const turnSound = new Audio('../Belote/sons/pop.mp3');
...
//let chessPath = ""; // Variable globale pour stocker le chemin d'accès à Chess, définie dans chess.js
let chessPath = "Chess/"; // Variable globale pour stocker le chemin d'accès à Chess, définie dans chess.js
...
function saveSession(rid, idx, name) {
    myRoomId = rid; myIndex = idx; myName = name;
...
function createGame() {
    ...
    fetch(`${chessPath}api.php?action=create&name=${encodeURIComponent(name)}`)
        .then(r=>r.json())
        .then(d => {
...
function joinGame(roomId = null) { 
    ...
    if(!inputName) { if(btn) btn.disabled = false; return showError("Veuillez choisir un pseudo !"); }
    if(!c) { if(btn) btn.disabled = false; return showError("Code manquant"); }

    fetch(`${chessPath}api.php?action=join&roomId=${c}&name=${encodeURIComponent(inputName)}`)
        .then(r=>r.json())
        .then(d => { 
            if(d.error) { if(btn) btn.disabled = false; return showError(d.error); }
...
function leaveLobby() {
    showConfirm("Quitter la partie ?", function() {
        if(myRoomId && myName) {
            fetch(`${chessPath}api.php?action=leave&roomId=${myRoomId}&name=${encodeURIComponent(myName)}`)
                .finally(() => {
...
function launchGame() { 
    ...
    fetch(`${chessPath}api.php?action=startRound&roomId=${myRoomId}`); 
}

function cancelGame() { 
    showConfirm("Annuler la partie ?", () => { 
        fetch(`${chessPath}api.php?action=cancelGame&roomId=${myRoomId}`); 
    }); 
}
