...
function enterLobby(rid, admin) { 
    ...
    scoreSel.onchange = function() { fetch(`api.php?action=updateSettings&roomId=${myRoomId}&param=${this.value}`); };
...
function startPolling() {
    setInterval(() => {
        ...
        fetch(`api.php?action=get&roomId=${myRoomId}`)
            .then(r => r.json())
            .then(data => {
                ...
                if (data.admin && myName) {
                    ...
                    const sl = document.getElementById('lobby-param-XXXX'); if(sl) sl.disabled = !isAdmin;
                }
...
function refreshRoomList() {
    ...
    fetch('api.php?action=listRooms')
        .then(r => r.json())
        .then(data => {
            if (!data.success) return;
            
            if (data.rooms.length === 0) {
                container.innerHTML = '<div class="no-rooms">Aucun salon disponible.<br>Créez le vôtre !</div>';
                return;
            }
...
function continueGame() {
    ...
    fetch(`api.php?action=nextTrick&roomId=${myRoomId}`)
        .then(() => {
            document.getElementById('score-modal').style.display = 'none';
        });
}
...
function renderGame(data) {
    // Sécurité index
    if (myIndex === null) {
        data.players.forEach((p, i) => { if(p.name === myName) myIndex = i; });
    }
    ...
    data.table.forEach(play => {
        ...
        let relativeIndex = (play.playerIndex - myIndex + playerCount) % playerCount;
...
function launchGame() {
    fetch(`api.php?action=startRound&roomId=${myRoomId}`);
}

function playCard(cid) {
    fetch(`api.php?action=play&roomId=${myRoomId}&index=${myIndex}&cardId=${cid}`);
}

function backToLobby() {
    showConfirm("Retourner au salon ? (Cela réinitialisera les scores)", () => {
        fetch(`api.php?action=backToLobby&roomId=${myRoomId}`);
    });
}
