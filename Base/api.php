 ...
 */
function processRoom($roomId, $callback) { global $dataDir; $file = $dataDir . 'room_' . preg_replace('/[^A-Z0-9]/', '', $roomId) . '.json'; $fp = fopen($file, 'c+'); if (!$fp) exit(json_encode(['error' => 'Erreur fichier'])); if (flock($fp, LOCK_EX)) { $size = filesize($file); $content = $size > 0 ? fread($fp, $size) : ''; $json = $content ? json_decode($content, true) : null; $result = $callback($json); if ($result !== null) { ftruncate($fp, 0); rewind($fp); $result['lastUpdate'] = time(); fwrite($fp, json_encode($result)); } flock($fp, LOCK_UN); } else { exit(json_encode(['error' => 'Occupé'])); } fclose($fp); }
...
switch ($action) {
    ...
    case 'create':
        ...
        if(!$name) exit(json_encode(['error' => 'Pseudo vide']));
    ...
    case 'join':
        ...
        processRoom($roomId, function($json) use ($name) {
            if (!$json) return null;
            if (count($json['players']) >= 4) { echo json_encode(['error' => 'Salon complet (Max 4)']); return null; }
            foreach ($json['players'] as $p) if ($p['name'] === $name) { echo json_encode(['error' => 'Pseudo déjà pris']); return null; }
    ...
    case 'startRound':
        $roomId = $_REQUEST['roomId'];
        processRoom($roomId, function($json) {
            if (count($json['players']) < 2) { echo json_encode(['error' => 'Pas assez de joueurs']); return null; }
    ...
    case 'play':
        ...
        processRoom($roomId, function($json) use ($cardId, $idx) {
            ...
            if ($json['turnIndex'] !== $idx) { echo json_encode(['error' => 'Ce n\'est pas votre tour !']); return null; }
            ...
            foreach($hand as $k => $c) { 
                if($c['id'] === $cardId) { $cardPlayed = $c; $cardIndex = $k; break; } 
            }
            
            if (!$cardPlayed) { echo json_encode(['error'=>'Carte introuvable']); return null; }
    ...
    case 'nextTrick':
        $roomId = $_REQUEST['roomId'];
        processRoom($roomId, function($json) {
            if ($json['status'] !== 'round_end') return null;
    ...
    case 'leave':
        $roomId = $_REQUEST['roomId']; $name = $_REQUEST['name'];
        processRoom($roomId, function($json) use ($name, $dataDir, $roomId) {
            $json['players'] = array_values(array_filter($json['players'], function($p) use ($name) { return $p['name'] !== $name; }));
            if (empty($json['players'])) { 
                if(file_exists($dataDir.'room_'.$roomId.'.json')) unlink($dataDir.'room_'.$roomId.'.json'); 
                echo json_encode(['success'=>true]); return null; 
            }
            if ($json['admin'] === $name) $json['admin'] = $json['players'][0]['name'];
            echo json_encode(['success' => true]); return $json;
        }); 
        break;
    ...
    case 'backToLobby':
        ...
        processRoom($roomId, function($json) {
            $json['status'] = 'lobby';
            foreach ($json['players'] as &$p) $p['hand'] = [];
            $json['table'] = [];
            $json['turnIndex'] = 0;
            echo json_encode(['success' => true]); 
            return $json;
